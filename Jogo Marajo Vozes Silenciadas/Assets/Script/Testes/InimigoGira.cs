...
public class InimigoGira : MonoBehaviour
{
    ...
    IEnumerator GirarSuavemente(float anguloDe, float anguloPara, float duracao)
    {
        ...
        while (tempoPassado < duracao)
        {
            ...
            Vector2 direcaoAtual = AnguloParaVetor(transform.eulerAngles.z);
            campoVisao.DirecaoInimigoN(-direcaoAtual);
         
            yield return null; 
        }
        
        transform.rotation = Quaternion.Euler(0f, 0f, anguloPara);

        Vector2 direcaoFinal = AnguloParaVetor(transform.eulerAngles.z);
